...
def merge_data(df_1, df_2):
    ...
    combined_data.rename(columns={'# Affected Cases in Cohort_x': f'{location1} Frequency', '# Affected Cases in Cohort_y': f'{location2} Frequency', 'Impact_y': 'Impact'}, inplace=True)
    combined_data[['Fraction', f'{location1} Percentage']] = combined_data[f'{location1} Frequency'].str.split(',', expand=True)
    combined_data[['Fraction2', f'{location2} Percentage']] = combined_data[f'{location2} Frequency'].str.split(',', expand=True)
    combined_data.drop([f'{location1} Frequency', f'{location2} Frequency', 'Fraction', 'Fraction2'], axis=1, inplace=True)
